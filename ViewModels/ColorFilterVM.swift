...
class FrameHandler: NSObject, ObservableObject {
    ...
    private let context = CIContext()
     private var centerColor: (red: UInt8, green: UInt8, blue: UInt8)? = nil
    private var filterColor: (red: UInt8, green: UInt8, blue: UInt8)? = nil
...
//    @Published var filterIntensity:Float = 0.9
    ...
    func setColor() {
        ...
        useFilter = true
//        print("Center pixel RGB: R: \(filterColor!.red), G: \(filterColor!.green), B: \(filterColor?.blue)")
    }
...
//    private func applyFilter(to image: CIImage, intensity: Float) -> CIImage? {
...
//    nonisolated func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) async {
//        // Convert the sample buffer to a CIImage
//        guard let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) else { return }
//        let cameraImage = CIImage(cvPixelBuffer: pixelBuffer)
//
//        // Render the filtered image to a CGImage in the nonisolated context
//        let context = CIContext()
//        guard let filteredCGImage = await self.createFilteredCGImage(from: cameraImage, usingContext: context) else { return }
//
//        // Update the frame on the MainActor with the filtered UIImage
//        Task { @MainActor in
//            self.frame = UIImage(cgImage: filteredCGImage)
//        }
//    }
//
//    private func createFilteredCGImage(from image: CIImage, usingContext context: CIContext) async -> CGImage? {
//        // Copy the filterIntensity for thread safety
//        let intensity: Float = await Task { @MainActor in
//            self.filterIntensity
//        }.value
//
//        // Apply the filter
//        let filteredImage = self.applyFilter(to: image, intens: intensity) ?? image
//
//        // Render the filtered CIImage to a CGImage
//        return context.createCGImage(filteredImage, from: filteredImage.extent)
//    }
//
//    private func applyFilter(to image: CIImage, intens: Float) -> CIImage? {
//        let filter = CIFilter.sepiaTone()
//        filter.inputImage = image
...
//        
//        let cameraImage = CIImage(cvPixelBuffer: pixelBuffer)
//        let filteredImage = applyFilter(to: cameraImage) ?? cameraImage
// 
//        let context = CIContext()
//        if let cgImage = context.createCGImage(filteredImage, from: filteredImage.extent) {
//            let imageToPublish = UIImage(cgImage: cgImage)
//            
//            Task { @MainActor in
//                
//                self.frame = imageToPublish
